/**
 * Every country, with its dial code and ISO 3166-1 alpha-2 code.
 *
 ...
 *
 * **Sorted by name, not by code.** `+1` is Canada, the US and eighteen
 * Caribbean nations; ordering by dial code interleaves them and there is no
 * sensible place to put the duplicates. Names are unique and are what somebody
...
/**
 * Country names for an address `<select>`, in the same order as the list above
 * but with Canada lifted to the top: it is the answer for almost every account,
 * and making the common case the first row is worth breaking the sort for.
 */
export const COUNTRY_OPTIONS = [
  { value: DEFAULT_COUNTRY, label: DEFAULT_COUNTRY },
  ...COUNTRIES.filter((entry) => entry.name !== DEFAULT_COUNTRY).map((entry) => ({
    value: entry.name,
    label: entry.name,
  })),
];
