...
const TYPE_AREA = {
  ...
  // Same area and the same reasoning as an application: they arrive from
  // outside, and the purchasing team is the only one who acts on them. Without
  // a bell, a price or a proforma sits unread until somebody happens to reopen
  // the order - and a delivery marked dispatched is the cue to expect stock.
  po_quoted: 'purchase',
  po_proforma: 'purchase',
  po_delivery: 'purchase',
  new_order: 'sales',
...
async function derivedFor(areas, now) {
  ...
  if (areas.has('purchase')) {
    jobs.push(
      /**
       * **One row for the whole reorder queue, not one per empty shelf.**
       *
       * This used to emit a notification per product, which on a real
       * catalogue meant forty rows that between them said one thing - *stock
       * needs buying* - and pushed every other alert off the panel. Worse,
       * none of them could be acted on: an empty shelf is cleared by raising a
       * purchase order, and there was no way to do that from here, so the rows
       * were forty restatements of a fact with no door out.
       *
       * So it collapses to a single row carrying the counts and the action
       * that actually resolves them. The per-product detail did not disappear
       * - it is the Inventory screen, which this row links to, and which shows
       * far more about each product than a one-line alert ever could.
       *
       * The queue is computed by `reorderService`, which is also what the
       * action and the Inventory screen use, so the number on the bell and the
       * number on the screen it opens can no longer disagree.
       */
      getReorderQueue()
        .then(({ counts }) => {
          if (!counts.total) return [];

          const { out, low, total } = counts;
          const parts = [];
          if (out) parts.push(`${out} out of stock`);
          if (low) parts.push(`${low} below reorder point`);
          ...
          return [
            {
              ...
              detail: parts.join(' · '),
              entity: { kind: 'reorder', id: 'queue', label: 'Reorder queue' },
              href: '/admin/inventory?stock=attention',
              area: 'purchase',
              read: false,
              // No moment of onset in the records, so dated to now purely to
              // sort. `standing` stops the client rendering that as "just now".
              createdAt: now,
              standing: true,
              /**
               * The second row type in this panel that can be acted on from
               * here, and for the same reason approvals can: the alert *is* the
               * decision. Everything else on the bell is cleared by work done
               * elsewhere, so a button on those rows would have nothing to
               * press; this one opens the purchase order that answers it.
               *
               * **It opens a form, it does not raise an order.** The counts came
               * off reorder points a machine read, so the quantities want a
               * person before any record exists - the client navigates to the
               * create screen with the lines filled in, and nothing is written
               * until somebody saves it. `label` says "Order stock" rather than
               * "Draft PO" for that reason: the button opens a screen, and a
               * label naming a document implies one is about to exist.
               */
              action: {
                kind: 'generate_reorder_draft',
                label: 'Order stock',
                outCount: out,
                lowCount: low,
                total,
